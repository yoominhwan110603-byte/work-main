  ...
  <div v-if="album" class="size-full bg-white flex flex-col">
    <header class="px-4 py-4 flex items-center justify-between border-b sticky top-0 bg-white z-10">
      ...
      <div class="flex gap-2">
        <button class="p-2" aria-label="공유" @click="shareAlbum"><Share2 :size="22" /></button>
      ...
      <div class="p-4 space-y-4">
        ...
        <section class="border-t pt-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div><p class="text-sm text-gray-600 mb-1">표면 확인</p><p class="text-lg">{{ album.audioScore >= 85 ? '양호' : '주의' }}</p></div>
          </div>
          </div>
        </section>
        ...
        <p v-if="shareMessage" class="text-xs text-center text-blue-600">{{ shareMessage }}</p>
      </div>
    </div>
...
import { ArrowLeft, BadgeCheck, ClipboardList, Eye, Flag, Heart, MapPin, MessageCircle, Pencil, Play, Share2, Volume2 } from 'lucide-vue-next';
import { getActiveTrade } from '../data/tradeState';
import { useAppStore } from '../stores/appStore';
import { findAlbumById } from '../data/albumLookup';
import { makeOneToOneChatId } from '../data/chatClient';
import { goBackOr } from '../data/navigation';
...
const shareAlbum = async () => {
  ...
  const url = `${window.location.origin}${window.location.pathname}#/app/album/${album.value.id}`;
  const title = `${album.value.title} - ${album.value.artist}`;
  const text = `${title}\n${album.value.price.toLocaleString()}원`;
  try {
    if (navigator.share) {
    ...
    shareMessage.value = '공유 링크를 복사했습니다.';
  } catch {
    shareMessage.value = '공유를 완료하지 못했습니다.';
  } finally {
    window.setTimeout(() => { shareMessage.value = ''; }, 2400);
  }
};
