<template>
  <div class="size-full bg-black text-white flex flex-col">
    ...
    <section v-if="qualityResult" class="mx-4 mb-3 rounded-2xl bg-white p-4 text-gray-900">
      ...
      <button class="mt-3 w-full rounded-lg bg-blue-600 py-3 text-sm text-white" @click="applyCapture">
        판매 화면에 적용
      </button>
    </section>
    ...
    <footer class="p-6 flex items-center justify-center gap-8">
      ...
      <button
        :class="['w-20 h-20 rounded-full flex items-center justify-center active:scale-95 transition', isRecording ? 'bg-red-600 text-white' : 'bg-white text-black']"
...
import { setPendingCapture } from '../data/captureTransfer';
...
const capturePhoto = () => {
  ...
  pendingDataUrl.value = canvas.toDataURL('image/jpeg', 0.92);
...
const pickFallback = async (event: Event) => {
  ...
  if (!file) return;
  pendingDataUrl.value = await readFileAsDataUrl(file);
  qualityResult.value = {
    score: 70,
