<template>
  <div class="size-full bg-white flex flex-col">
    <header class="px-4 py-4 flex items-center border-b"><button class="p-2" @click="goBack(router)"><ArrowLeft :size="24" /></button><h1 class="ml-4 text-lg">거래 취소</h1></header>
    ...
    <div class="p-4 border-t flex gap-2"><button class="flex-1 py-4 border border-gray-300 rounded-xl" @click="goBack(router)">돌아가기</button><button class="flex-1 py-4 bg-red-600 text-white rounded-xl disabled:bg-gray-300" :disabled="!selectedReason || (selectedReason === '기타' && !otherReason)" @click="submit">거래 취소</button></div>
  </div>
</template>
