<template>
  <div class="size-full bg-black text-white flex flex-col">
    <header class="px-4 py-4 flex items-center justify-between z-10">
      <button class="p-2 rounded-full bg-white/10 active:bg-white/20" @click="goBackOr(router, returnPath)">
        <ArrowLeft :size="24" />
      </button>
      <div class="text-right">
        <p class="text-sm font-medium">{{ isCoverMode ? '대표 이미지 촬영' : isVideoMode ? '스크래치 동영상 촬영' : '표면 스캔' }}</p>
        <p class="text-xs text-white/60">{{ isCoverMode ? '앨범 커버 전체를 프레임 안에 맞춰주세요' : 'LP판 전체를 원 안에 맞춰주세요' }}</p>
      </div>
    </header>
    ...
    <main class="flex-1 flex items-center justify-center px-4">
      <div class="relative w-full max-w-md aspect-square overflow-hidden rounded-2xl bg-neutral-950">
        ...
        <img v-if="pendingDataUrl && !isVideoMode" :src="pendingDataUrl" :alt="isCoverMode ? '촬영한 대표 이미지' : '촬영한 표면 이미지'" class="absolute inset-0 h-full w-full object-contain" />
        ...
        <div :class="['pointer-events-none absolute inset-[7%] z-10 border-2 border-white/95 shadow-[0_0_0_999px_rgba(0,0,0,0.3)]', isCoverMode ? 'rounded-lg' : 'rounded-full']"></div>
        <div v-if="!isCoverMode" class="pointer-events-none absolute inset-[16%] z-10 rounded-full border border-dashed border-white/55"></div>
        <div v-if="!isCoverMode" class="pointer-events-none absolute inset-0 z-10 flex items-center justify-center">
          <div class="h-14 w-14 rounded-full border border-white/65 bg-black/15"></div>
        </div>

        <div v-if="!isCoverMode" class="pointer-events-none absolute left-4 top-20 z-10 flex items-center gap-2 rounded-full bg-sky-500/90 px-3 py-2 text-xs font-medium text-white shadow-lg">
          <span class="h-px w-12 bg-white"></span>
          <span class="text-lg leading-none">→</span>
          <span>반사광 방향</span>
        </div>
        <div v-if="!isCoverMode" class="pointer-events-none absolute left-6 top-32 z-10 h-20 w-28 rotate-[-18deg] border-t-2 border-l-2 border-sky-300/90"></div>
        ...
        <div class="pointer-events-none absolute left-5 right-5 top-4 z-10 rounded-full bg-black/65 px-4 py-2 text-center text-sm font-medium">
          {{ isCoverMode ? '앨범 커버 전체가 보이게 촬영하세요' : 'LP판 전체가 원 안에 들어오게 촬영하세요' }}
        </div>
        <div class="pointer-events-none absolute left-5 right-5 bottom-20 z-10 space-y-2">
          <p v-if="!isCoverMode" class="rounded-full bg-black/70 px-4 py-2 text-center text-xs text-white">
        ...
        <div :class="['absolute left-3 right-3 bottom-3 z-10 grid gap-2 text-[11px]', isCoverMode ? 'grid-cols-2' : 'grid-cols-3']">
          <div :class="metricClass(brightnessState)">
            밝기 {{ Math.round(metrics.brightness) }}
          </div>
          <div v-if="!isCoverMode" :class="metricClass(reflectionState)">
    ...
    <section v-if="qualityResult" class="mx-4 mb-3 rounded-2xl bg-white p-4 text-gray-900">
      <p v-if="isCoverMode" class="text-sm font-medium">대표 이미지 확인</p>
      ...
      <div v-if="!isCoverMode" class="mt-3 grid grid-cols-4 gap-2 text-center text-[11px]">
      ...
      <button class="mt-3 w-full rounded-lg bg-blue-600 py-3 text-sm text-white disabled:bg-gray-400" :disabled="isApplyingCapture" @click="applyCapture">
        {{ isApplyingCapture ? '적용 중' : isCoverMode ? '대표 이미지에 적용' : isVideoMode ? '표면 동영상에 적용' : '표면 이미지에 적용' }}
      </button>
    </section>
...
const isCoverMode = computed(() => !isVideoMode.value && route.query.target === 'cover');
const returnPath = computed(() => {
  const path = typeof route.query.returnTo === 'string' ? route.query.returnTo : '';
  return /^\/app\/sell(?:\/[^/]+\/edit)?$/.test(path) ? path : '/app/sell';
});
...
const primaryGuide = computed(() => {
  if (!isCoverMode.value && metrics.value.fit < 35) return 'LP판이 잘리고 있어요. 원 안에 다시 맞춰주세요';
  if (metrics.value.brightness < 35) return '화면이 너무 어두워요';
  if (metrics.value.brightness > 218) return '화면이 너무 밝아요';
  if (!isCoverMode.value && metrics.value.reflection < 14) return '반사가 부족해요. 조명 각도를 조정해주세요';
  if (metrics.value.motion > 22) return '너무 빠르게 움직이고 있어요';
  return '좋아요. 이 각도를 유지하세요';
});
const warningText = computed(() => {
  if (!cameraStream || cameraMessage.value) return '';
  if (!isCoverMode.value && metrics.value.fit < 35) return 'LP판 전체가 원 안에 들어오게 다시 맞춰주세요';
  if (metrics.value.motion > 22) return '너무 빠르게 움직이고 있어요';
  if (metrics.value.brightness < 35) return '화면이 너무 어두워요';
  if (metrics.value.brightness > 218) return '화면이 너무 밝아요';
  if (!isCoverMode.value && metrics.value.reflection < 14) return '반사가 부족해요. 조명 각도를 조정해주세요';
...
const applyCapture = async () => {
  ...
  await setPendingCapture(isVideoMode.value ? 'video' : 'image', pendingDataUrl.value, isCoverMode.value ? 'cover' : 'record');
  clearTimers();
  stopCamera();
  const previous = router.options.history.state.back;
  if (typeof previous === 'string' && router.resolve(previous).path === returnPath.value) router.back();
  else await router.replace(returnPath.value);
};
