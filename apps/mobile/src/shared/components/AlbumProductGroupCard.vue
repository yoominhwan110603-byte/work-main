  ...
  <article class="rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
    ...
    <div class="mt-3 divide-y rounded-lg border border-gray-100">
      ...
      >
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2">
            <span class="truncate text-sm font-medium text-gray-950">{{ pressing.catalogNumber }}</span>
      ...
      <div v-if="hiddenPressingCount > 0" class="px-3 py-2 text-xs text-gray-400">
        카탈로그 {{ hiddenPressingCount }}개 더 있음
      </div>
    </div>
  </article>
</template>
...
import type { AlbumProductGroup, PressingGroup } from '@/features/buyer/services/pressingCatalog';
