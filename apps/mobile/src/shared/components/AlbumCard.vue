    ...
    <div class="flex gap-3">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex flex-wrap items-center gap-1.5 mb-2">
          ...
          <span v-for="tag in visibleTags" :key="tag" class="px-2 py-0.5 bg-gray-100 text-gray-700 text-xs rounded">
            {{ displayTag(tag) }}
          </span>
        </div>
...
const visibleTags = computed(() => (props.album.tags || [])
  .filter(Boolean)
  .filter(tag => !tag.includes('희귀') && !tag.toLowerCase().includes('rare'))
  .slice(0, 2));
